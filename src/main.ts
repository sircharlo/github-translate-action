...
async function main(): Promise<void> {
  ...
  const originalBodyWithQuotes = body?.split(TRANSLATION_DIVIDER)?.[0];
  ...
  if (appendTranslation) {
    const finalTitle =
      ...
      originalTitle &&
      `${originalTitle}${TITLE_DIVIDER}${formatTitleWithTags(originalTitle, translatedTitle)}`;
...
> ${botNote}
----
${!modifyTitle && translatedTitle && originalTitle ? `**Title:** ${formatTitleWithTags(originalTitle, translatedTitle)}\n` : ''}
${translatedBody ?? ''}
`.trim();
